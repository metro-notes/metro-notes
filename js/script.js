...
var insertNote = function(noteObj, index) {
	...
	note.children('#handle-' + index).append('<img src="' + chrome.extension.getURL('icons/move.png') + '" class="handle" title="Move this note." />');
	note.children('#delete-' + index).append('<img src="' + chrome.extension.getURL('icons/delete.png') + '" class="trash" title="Delete this note." />');
	...
	if(url.length) {
		chrome.extension.sendMessage({cmd: 'loadNotes', data: {url: url }}, function(newNotes) {
			...
			if(newNotes !== null) {
				...
				notes = newNotes;
				chrome.extension.sendMessage({cmd: 'badge', data: {count: notes.length }});
			}
		});
	}
}
...
var init = function() {
	...
	overlayObject.append('<div class="options toggle-key-label" id="toggle-key-label">toggle key<span id="toggle-key"></span></div>');
	overlayObject.append('<div class="options" id="reset">reset</div>');
